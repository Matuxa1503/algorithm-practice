...
function formatTime(minutes, seconds) {
  const num1 = `${String(minutes).padStart(2, '0')}:`;
  const num2 = String(seconds).padStart(2, '0');
...
function isPalindrome(str) {
  const curStr = str.toLowerCase().replace(/[^a-z\d]/gi, '');
  const backStr = curStr.split('').reverse().join('');
  if (curStr === backStr) {
    return true;
  }
  return false;
...
function getCardId(value) {
  ...
  switch (value.slice(value.length - 1)) {
    case '♣':
      ind = str.indexOf(value[0]);
      break;
    case '♦':
      ind = str.indexOf(value[0]) + 13;
      break;
    case '♥':
      ind = str.indexOf(value[0]) + 26;
      break;
    default:
      ind = str.indexOf(value[0]) + 39;
  }
  return ind;
}
